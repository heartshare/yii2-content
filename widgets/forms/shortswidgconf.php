...
<?php     $form = ActiveForm::begin(
    [
        ...
        'method' => 'post',
        'action' => \yii\helpers\Url::toRoute(['/content/widget/shorts-widget-configure', 'id' => $wmodel->id])
    ]
);
?>
<?php
...
<?= $form->field($model, 'showanons')->dropDownList([0 => 'Нет', 1 => 'Да'])->label('Показывать текст')->hint('Если нет, то будет отображен только заголовок'); ?>
<?=
$form->field($model, 'anonslimit')->textInput([])->label('Кол-во символов текста')->hint(
    '(Если 0, текст обрезаться не будет)'
); ?>
