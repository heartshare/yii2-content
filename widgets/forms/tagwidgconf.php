...
<?php     $form = ActiveForm::begin(
    [
        ...
        'method' => 'post',
        'action' => \yii\helpers\Url::toRoute(['/content/widget/tag-widget-configure', 'id' => $wmodel->id])
    ]
);
?>
<?php
...
<?= $form->field($model, 'limit')->textInput([])->label('Кол-во тегов'); ?>
<?=
$form->field($model, 'sorttype')->dropDownList(
    [
        \insolita\content\widgets\TagWidget::SORT_ALPHA => 'По алфавиту',
        \insolita\content\widgets\TagWidget::SORT_FREEQ => 'По чатсоте'
    ]
)->label('Сортировка тегов'); ?>
<?=
$form->field($model, 'viewtype')->dropDownList(
    [
        \insolita\content\widgets\TagWidget::VIEW_CLOUD => 'В строчку',
