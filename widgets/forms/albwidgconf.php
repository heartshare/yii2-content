...
<?php     $form = ActiveForm::begin(
    [
        ...
        'method' => 'post',
        'action' => \yii\helpers\Url::toRoute(['/content/widget/album-widget-configure', 'id' => $wmodel->id])
    ]
);
?>
<?php
...
$form->field($model, 'sorttype')
    ->dropDownList(
        [
            \insolita\content\widgets\AlbumWidget::SORT_RAND => 'Случайные',
           \insolita\content\widgets\AlbumWidget::SORT_LATEST => 'Самые новые',
        ]
    )
    ->label('Тип сортировки');?>
<?= $form->field($model, 'total')->textInput([])->label('Количество альбомов'); ?>
